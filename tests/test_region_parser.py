...
class TestRegionFileParserToDict:
    ...
    def test_to_dict_with_container(self, tmp_path):
        ...
        assert "37,64,74" in result
        assert result["37,64,74"]["name"] == "Container"
        assert result["37,64,74"]["components"]["container"]["capacity"] == 18
    ...
    def test_to_dict_with_block_component(self, tmp_path):
        ...
        assert "10,32,15" in result
        assert result["10,32,15"]["name"] == "FarmingBlock"
        assert "FarmingBlock" in result["10,32,15"]["components"]
    ...
    def test_to_dict_multiple_chunks(self, tmp_path):
        ...
        assert "0,10,0" in result  # From chunk (0, 0)
        assert "32,20,0" in result  # From chunk (1, 0): 1*32 + 0 = 32
